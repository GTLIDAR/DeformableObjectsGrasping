...
class VIVIT(nn.Module):
    def __init__(self,Tactile_img_size, Visual_img_size, Tactile_patch_size, Visual_patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio,qkv_bias,qk_scale, drop_rate, attn_drop_rate, drop_path_rate,num_frames,dropout, use_gpu=False):
        ...
        #form the batch
        # x -> visual_pinching image
        # y -> tactile_pinching image
        # z -> visual_sliding image
        # w -> tactile_sliding image
        ...
        self.patch_embed_z = UniformPatchEmbed(img_size=Visual_img_size,patch_size=Visual_patch_size,in_chans=in_chans,embed_dim=embed_dim)
        num_patches_z = self.patch_embed_z.num_patches
        self.patch_embed_w = UniformPatchEmbed(img_size=Tactile_img_size,patch_size=Tactile_patch_size,in_chans=in_chans,embed_dim=embed_dim)
        num_patches_w = self.patch_embed_w.num_patches

        if self.use_gpu == True:
            self.patch_embed_x.to('cuda')
            self.patch_embed_y.to('cuda')
            self.patch_embed_z.to('cuda')
            self.patch_embed_w.to('cuda')
    ...
    def forward_features_z(self,x):
        B = x.shape[0]
        x, T, W = self.patch_embed_z(x)
        H = x.size(1) // W
        #cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        #x = torch.cat((cls_tokens, x), dim=1)
    ...
    def forward_features_w(self,x):
        B = x.shape[0]
        x, T, W = self.patch_embed_w(x)
        H = x.size(1) // W
        #cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        #x = torch.cat((cls_tokens, x), dim=1)

        x = x + self.pos_embed_w
        x = self.pos_drop(x)

        ## Attention blocks
        for blk in self.blocks_w:
            x = blk(x, B, T, W)
    ...
    def forward(self,x1,x2,y1,y2,thresh):
        ...
        x2 = self.forward_features_z(x2) 
        y1 = self.forward_features_y(y1)
        y2 = self.forward_features_w(y2)
