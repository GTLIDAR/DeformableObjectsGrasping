...
class Basic_network(nn.Module):  # after fc, 64 values
    ...
    def forward(self, x_1, x_2):  #input two images at a time (fusion)
        features_1 = self.features(x_1)
        features_2 = self.features(x_2)
        features = torch.cat((features_1, features_2), 1)  # concatenate the two rows  1 -> add along the column
        features = features.view(features.size(0), -1)   #return a tensor of a different shape  -1 > infer the other dimention, which means you can always find a correct shape
        features = self.fc(features)
        features = self.dropout_(features)
        return features # (64,1) tensor for LSTM model
...
class RNN_network(nn.Module):
    def __init__(self, input_size=64, hidden_size=64, num_layers=2, num_classes=2, use_gpu=False, dropout=0.8):
        ...
        self.fc = nn.Linear(hidden_size, num_classes)  # project to 2-label data
...
class Basic_CNN(nn.Module):
    def __init__(self, base_network='vgg_16', pretrained=False, rnn_input_size=64, rnn_hidden_size=64,
                 rnn_num_layers=1, num_classes=2, use_gpu=False, frozen_weights = False, dropout_CNN=0.5, dropout_LSTM=0.8, video_length = 8):  #classes -> slip or not
        ...
        self.rnn_network = RNN_network(input_size=rnn_input_size, hidden_size=rnn_hidden_size, num_layers=rnn_num_layers,
                                       num_classes=num_classes, use_gpu=use_gpu, dropout=dropout_LSTM)
    ...
    def forward(self, x_1, x_2):
        ...
        for i in range(self.video_length):   # get 8 * (64, 1) tensor (a sequence and then fed into LSTM)
            if self.use_gpu:
                features = self.cnn_network(x_1[:,:,i,:,:].to('cuda'), x_2[:,:,i,:,:].to('cuda'))
            else:
