...
class Basic_network(nn.Module):  # after fc, 64 values
    def __init__(self, base_network='vgg_16', pretrained = False, frozen_weights = False, dropout = 0.5, rnn_input_size = 64):
        ...
        if base_network == 'vgg_16':
            ...
            self.fc = nn.Sequential(nn.Linear(4096, rnn_input_size))  # *2 -> combine tactile & visual (this one should be trained)
            # RNN input size: 64
            ...
            for param in self.features.parameters():
                param.requires_grad = not frozen_weights
            self.fc = nn.Sequential(nn.Linear(4096, rnn_input_size))
    ...
    def forward(self, x_1):  #input two images at a time (fusion)
        features_1 = self.features(x_1)
        features = self.fc(features_1)
...
class Basic_CNN(nn.Module):
    ...
    def forward(self, x_1):
        ...
        """
        :param x_1: a list of 8 imgs(tensor)
        :return: network output
        """
        for i in range(self.video_length):   # get 8 * (64, 1) tensor (a sequence and then fed into LSTM)
            if self.use_gpu:
                features = self.cnn_network(x_1[:,:,i,:,:].to('cuda'))
            else:
                features = self.cnn_network(x_1[:,:,i,:,:])
            if i == 0:
