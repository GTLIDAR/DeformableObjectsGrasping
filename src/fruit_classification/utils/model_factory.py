...
sys.path.append("..")
from Models import vivit_FDP_two_fruit, basic_CNN, timeSformer_orig_two_fruit
...
def get_model(config_data,use_gpu):
    ...
    if model_type == 'vivit_fdp_two_fruit':
        ...
        in_chans = 3
        num_cls = 5 # 0 -> slippery; 1 -> safe; 2 -> damage
        emb_dim=256
        depth=8
        num_heads=16
        mlp_ratio=4
    ...
    elif model_type == 'timeSformer_orig_two_fruit':
        Tactile_img_size  = (int(Tactile_resized_height),int(Tactile_resized_width))
        Visual_img_size  = (int(Visual_resized_height),int(Visual_resized_width))
        Tactile_patch_size = (15, 20)
        Visual_patch_size = (12, 16)
